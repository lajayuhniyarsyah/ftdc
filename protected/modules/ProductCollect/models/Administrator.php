...
class Administrator extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('modified_by','default','value'=>Yii::app()->getModule('ProductCollect')->user->getId(),'on'=>'insert'),
            array('created','default','value'=>new CDbExpression('NOW()')),
            array('username','unique'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, username, password, created, modified, created_by, modified_by', 'safe', 'on'=>'search'),
		);
	}
    ...
    private function hashPwd($str){
        return crypt($this->password,'34erdfcv');
    }
}